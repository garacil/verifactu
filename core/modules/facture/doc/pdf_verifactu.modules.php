...
/**
 *	Class to manage PDF invoice template verifactu
 */
	...
	public function write_file($object, $outputlangs, $srctemplatepath = '', $hidedetails = 0, $hidedesc = 0, $hideref = 0)
	{
		...
		if ($conf->facture->multidir_output[$conf->entity]) {
			...
			if (file_exists($dir)) {
				...
				$cert = $certprivate =  'file://' . __DIR__ . '/certificado.pem';


				// If a certificate is found
				if ($cert && $certprivate && file_exists($cert) && file_exists($certprivate)) {
					$info = array(
						'Name' => $this->emetteur->name,
						'Location' => getCountry($this->emetteur->country_code, 0),
						'Reason' => 'Certificado de calidad',
						'ContactInfo' => 'garacilb@gmail.com'
					);
					$pdf->setSignature($cert, $certprivate, $this->emetteur->name, '', 1, $info);
				}
				...
				$pdf->setSignatureAppearance(50, 500, 100, 100);
	...
	protected function drawTotalTable(&$pdf, $object, $deja_regle, $posy, $outputlangs, $outputlangsbis)
	{
		...
		if (empty($conf->global->MAIN_GENERATE_DOCUMENTS_WITHOUT_VAT)) {
			...
			if (!empty($conf->global->MAIN_GENERATE_DOCUMENTS_WITHOUT_VAT_IFNULL) && $tvaisnull) {
				...
				if ((int) $conf->global->INVOICE_USE_SITUATION == 1 && $object->situation_cycle_ref && $object->situation_counter > 1) {
					$sum_pdf_tva = 0;
				...
				foreach ($this->tva_array as $tvakey => $tvaval) {
					if ($tvakey != 0) {    // On affiche pas taux 0
						...
						$totalvat .= ' ';
						if (!empty($conf->global->PDF_VAT_LABEL_IS_CODE_OR_RATE) && $conf->global->PDF_VAT_LABEL_IS_CODE_OR_RATE == 'rateonly') {
							$totalvat .= vatrate($tvaval['vatrate'], 1) . $tvacompl;
						} elseif (!empty($conf->global->PDF_VAT_LABEL_IS_CODE_OR_RATE) && $conf->global->PDF_VAT_LABEL_IS_CODE_OR_RATE == 'codeonly') {
