...
/**
 *	\file       verifactu/views/list.facture.php
 *	\ingroup    verifactu
 *	\brief      Lista de facturas con gestión VeriFactu (basado en compta/facture/list.php)
 */
...
if (versioncompare(explode('.', DOL_VERSION), array(16)) < 0) {
	accessForbidden($langs->trans('VerifactuRequire16orHigher'));
}
//Verifica si la extension de php soap esta instalada
...
$arrayfields = array(
	...
	'f.ref_client' => array('label' => "RefCustomer", 'checked' => 0, 'position' => 10),
	//'f.type'=>array('label'=>"Type", 'checked'=>0, 'position'=>15),
	'f.datef' => array('label' => "DateInvoice", 'checked' => 1, 'position' => 20, 'type' => 'date'),
	'f.date_valid' => array('label' => "DateValidation", 'checked' => 0, 'position' => 22, 'type' => 'date'),
	'f.date_lim_reglement' => array('label' => "DateDue", 'checked' => 0, 'position' => 25, 'type' => 'date'),
	'f.date_closing' => array('label' => "DateClosing", 'checked' => 0, 'position' => 30, 'type' => 'date'),
	...
	'f.total_ht' => array('label' => "AmountHT", 'checked' => 0, 'position' => 95, 'type' => 'price'),
	...
	'f.total_ttc' => array('label' => "AmountTTC", 'checked' => 1, 'position' => 130, 'type' => 'price'),
	...
	//'f.multicurrency_code'=>array('label'=>'Currency', 'checked'=>0, 'enabled'=>(!$conf->multicurrency->enabled ? 0 : 1), 'position'=>280),
	//'f.multicurrency_tx'=>array('label'=>'CurrencyRate', 'checked'=>0, 'enabled'=>(!$conf->multicurrency->enabled ? 0 : 1), 'position'=>285),
	//'f.multicurrency_total_ht'=>array('label'=>'MulticurrencyAmountHT', 'checked'=>0, 'enabled'=>(!$conf->multicurrency->enabled ? 0 : 1), 'position'=>290),
	//'f.multicurrency_total_vat'=>array('label'=>'MulticurrencyAmountVAT', 'checked'=>0, 'enabled'=>(!$conf->multicurrency->enabled ? 0 : 1), 'position'=>291),
	//'f.multicurrency_total_ttc'=>array('label'=>'MulticurrencyAmountTTC', 'checked'=>0, 'enabled'=>(!$conf->multicurrency->enabled ? 0 : 1), 'position'=>292),
	//'multicurrency_dynamount_payed'=>array('label'=>'MulticurrencyAlreadyPaid', 'checked'=>0, 'enabled'=>(!$conf->multicurrency->enabled ? 0 : 1), 'position'=>295),
	//'multicurrency_rtp'=>array('label'=>'MulticurrencyRemainderToPay', 'checked'=>0, 'enabled'=>(!$conf->multicurrency->enabled ? 0 : 1), 'position'=>296), // Not enabled by default because slow
	...
	'f.datec' => array('label' => "DateCreation", 'checked' => 0, 'position' => 500, 'type' => 'datetime'),
	'f.tms' => array('label' => "DateModificationShort", 'checked' => 0, 'position' => 502, 'type' => 'datetime'),
...
foreach ($object->fields as $key => $val) {
	...
	if (!empty($val['visible'])) {
		...
		if (array_key_exists($key, $arrayfields)) {
			$newkey = $key;
		} elseif (array_key_exists('t.' . $key, $arrayfields)) {
			$newkey = 't.' . $key;
...
// Extra fields
include dol_buildpath('/verifactu/core/tpl/extrafields_list_array_fields.tpl.php');

$object->fields = dol_sort_array($object->fields, 'position');
$arrayfields = dol_sort_array($arrayfields, 'position');

$conf->global->MAIN_VIEW_LINE_NUMBER_IN_LIST = 1;

/*
 * Actions
 */
/* var_dump([1 => GETPOST('EXECUTEVERIFACTU'), 2 => $toselect, 3 => $action]); */

if (in_array(GETPOST('EXECUTEVERIFACTU'), array('Alta', 'Mod', 'Baja')) && count($toselect) > 0 && $action == 'confirm_EXECUTEVERIFACTU' && $confirm == 'yes') {
	$actionVERIFACTU = GETPOST('EXECUTEVERIFACTU');
	$facturesID = $toselect;

	try {
		foreach ($facturesID as $factureID) {
			$staticFacture = new Facture($db);
			$res = $staticFacture->fetch($factureID);
			if ($res < 0) {
				continue;
			}

			//Si la factura no esta valida, no esta en estado 1 o 2, no se envia
			if ($staticFacture->statut != Facture::STATUS_VALIDATED && $staticFacture->statut != Facture::STATUS_CLOSED) {
				setEventMessage($langs->trans('INVALID_VERIFACTU_STATUS_TO_SEND', $staticFacture->ref), 'errors');
				continue;
			}

			// Validate invoice is not cancelled in VERIFACTU
			if (!empty($staticFacture->array_options['options_verifactu_estado'])) {
				$estadoVERIFACTU = $staticFacture->array_options['options_verifactu_estado'];
				// Check if status contains "Anulada" (can be HTML badge or text)
				if (strpos($estadoVERIFACTU, 'Anulada') !== false || strpos($estadoVERIFACTU, 'VERIFACTU_STATUS_ANULADA') !== false) {
					setEventMessage($langs->trans('VERIFACTU_ERROR_FACTURA_YA_ANULADA') . ' - ' . $staticFacture->ref, 'errors');
					continue;
				}
			}

			// Validación: La referencia no debe contener "PROV" (no provisional)
			if (strpos($staticFacture->ref, 'PROV') !== false) {
				setEventMessage($langs->trans('VERIFACTU_VALIDATION_INVOICE_PROVISIONAL') . ' - ' . $staticFacture->ref, 'errors');
				continue;
			}
...
if ($resql) {
	...
	llxHeader('', $langs->trans('CustomersInvoicesVERIFACTU'), 'EN:Customers_Invoices|FR:Factures_Clients|ES:Facturas_a_clientes');
 ...
 */
	...
	print_barre_liste($langs->trans('CustomersInvoicesVERIFACTU') . ' ' . ($socid > 0 ? ' ' . $soc->name : ''), $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, 'bill', 0, $newcardbutton, '', $limit, 0, 0, 1);
	...
	if ($massaction == 'makepayment') {
		...
		$formconfirm = $form->formconfirm($_SERVER["PHP_SELF"], $langs->trans('MakePaymentAndClassifyPayed'), $langs->trans('EnterPaymentReceivedFromCustomer'), 'makepayment_confirm', $formquestion, 'yes', 1, 280, 600);
		print $formconfirm;
	}
	...
	if ($conf->categorie->enabled && $user->rights->categorie->lire && ($user->rights->produit->lire || $user->rights->service->lire)) {
	...
	//Crea un tr y un td que ocupe todo el ancho de la tabla
	print '<tr class="liste_titre">';
	print '<td colspan="' . (count($arrayfields) + 1) . '" class="liste_titre">';
	print '<div class="center">';
	if (empty(GETPOST('EXECUTEVERIFACTU')) || (GETPOSTISSET('EXECUTEVERIFACTU')) && ($confirm == 'no' || empty($confirm))) {
		...
		print '<button name="EXECUTEVERIFACTU" value="Baja" class="butActionDelete">' . $langs->trans('VERIFACTU_BUTTON_SENDTO_BAJA') . '</button>';
	}
	print '</div>';
	print '</td>';
	print '</tr>';
	...
	if ($num > 0) {
		...
		while ($i < $last_num) {
			...
			// Load extrafields to check VERIFACTU status
			$facturestatic->fetch_optionals();

			// Check if invoice is cancelled in VERIFACTU
			$isAnulada = false;
			if (!empty($facturestatic->array_options['options_verifactu_estado'])) {
				$estadoVERIFACTU = $facturestatic->array_options['options_verifactu_estado'];
				if (strpos($estadoVERIFACTU, 'Anulada') !== false || strpos($estadoVERIFACTU, 'VERIFACTU_STATUS_ANULADA') !== false) {
					$isAnulada = true;
				}
			}
			...
			$disabled = $isAnulada ? ' disabled title="' . $langs->trans('VERIFACTU_ERROR_FACTURA_YA_ANULADA') . '"' : '';
			if (!$disabled) {
